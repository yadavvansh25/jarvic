...
from dotenv import dotenv_values  # Importing dot env_values to read environment variables from a .env file.
...
*** Reply in only English, even if the question is in Hindi, reply in both hindi and English.***
...
try:
    with open(r"Data\ChatLog.json", "r") as f:
        messages = load(f)  # Load existing messages or initialize as empty.
...
def ChatBot(Query):
    ...
    try:
        ...
        # Handle exceptions as needed, resetting the chat log.
        print(f"Error: {e}")
        with open(r"Data\ChatLog.json", "w") as f:
            dump([], f, indent=4)
